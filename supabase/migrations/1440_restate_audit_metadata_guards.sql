-- Section 1440: restate the audit metadata guards as single, self-contained
-- definitions (bug-05 regression-guard staleness, primary + secondary gap).
--
-- This migration consolidates the two restating migrations that previously lived
-- in 1440 (unknown-key allowlist) and 1450 (required-key set) into one file, so
-- both audit-metadata guard functions are re-stated together as the effective
-- latest definitions, each carrying its own START/END markers.
--
-- Rationale:
--   (1) Primary gap (unknown-key allowlist):
...
--   (2) Secondary gap (required-key set):
--       audit_metadata_has_missing_required_key_for_action was defined across a deep
...
--       key set and none carried any marker, so the required-key set had NO executable
...
--       `cargo test` undetected.
--
--   This migration restores the invariant "one file = the complete guard definition"
--   by re-stating, for BOTH guards, the full per-action set (all 38 actions) as the
--   effective latest definition, with the -- ACTION_ALLOWLIST_START/END and
--   -- REQUIRED_KEY_START/END markers preserved. `create or replace` is forward-
--   compatible with databases that already applied 1230/1330/1360/1380/1390/1400 and
--   supersedes their delegating versions in place. The _before_NNNN chain is left
--   intact (still referenced by superseded versions in already-applied environments);
--   1440 just makes the effective top definitions complete and marker-bearing.
--
...
--
-- This change does NOT alter any allowed or required key. Both sets are byte-identical
-- to their prior effective definitions (verified), so no ADR is required (sets are
-- unchanged); it only moves the effective latest definitions into a single, marker-
-- bearing file.
