-- Section 1360: monthly digest 監査を success+failure 両方記録できるようにする。
--
-- 背景: monthly_digest_generate / monthly_digest_verify はこれまで failure-only として
-- DB 側で success を invalid_rpc_input で拒否していた。一方 Rust 検証層
-- (src/audit/event/metadata/allowlists.rs, validate.rs) は既に success metadata
-- (start/end_sequence_no, entry_count, signature_key_version, digest_hash, verify_result)
-- を受理する設計になっており、両者が乖離していた。本マイグレーションは DB を Rust 実装へ
-- 整合させ、「処理の成功・失敗に関係なく全操作を audit に記録する」方針を完成させる。
--
-- 信頼境界: monthly_digest の success metadata は sequence 番号・件数・署名鍵バージョン・
-- digest hash・対象年月・source_event_at のみ。平文・データキー・マスターキー・JWT・
-- ciphertext は一切含まない。

-- 1. rpc_append_audit_event: monthly_digest_* の failure-only ガードを撤去する。
--    （ベース: migration 1230。差分は monthly_digest_* の result 制約ブロック削除のみ。）
...
begin
    ...
    -- monthly_digest_generate / monthly_digest_verify は success+failure を許可する
    -- （Section 1360 で failure-only ガードを撤去）。
...
comment on function public.rpc_append_audit_event(
    ...
    'Audit append RPC for non-write-path audit events and failure events. Section 1360 allows monthly_digest_generate/verify success in addition to failure.';

-- 2. audit_metadata_has_unknown_key_for_action: monthly_digest の allowlist を Rust
--    (allowed_metadata_keys) と一致させる。generate は success metadata キーを追加、
--    verify は verify_result を追加する。
--    （ベース: migration 1330。差分は monthly_digest_* の分岐のみ。）
...
begin
    ...
    case p_action
        ...
        when 'monthly_digest_generate' then
            -- success: digest 範囲メタデータ / failure: error_code。union で許可する
            -- （Rust allowed_metadata_keys と一致。result 別の必須性は missing/invalid で担保）。
...
is 'Returns true when audit metadata contains a key outside the per-action allowlist. Section 1360 adds monthly_digest success metadata keys.';

-- 3. audit_metadata_has_missing_required_key_for_action: monthly_digest を result 別に
--    Rust (required_metadata_keys) と一致させる。
--    （ベース: migration 1230。差分は monthly_digest_* の分岐のみ。）
...
is 'Returns true when audit metadata is missing a required key for the given action/result. Section 1360 adds result-aware monthly_digest required keys.';

-- 4. 値検証: monthly_digest の success metadata は generic loop で扱う
--    audit_metadata_has_invalid_value_for_action_before_1330 を更新する。
--    start/end_sequence_no・signature_key_version は既に正整数群、digest_hash は hex64、
--    target_year_month / source_event_at も既に検証済み。追加するのは entry_count(正整数)と
--    verify_result の語彙('valid'/'invalid')のみ。Rust validate.rs と一致させる。
--    （ベース: migration 1230 の本体。1330 wrapper はこの関数へ委譲するため自動で反映される。）
...
begin
    ...
    loop
        if v_key in ('version', 'old_key_version', 'new_key_version', 'batch_size', 'target_sequence_no', 'start_sequence_no', 'end_sequence_no', 'entry_count', 'signature_key_version', 'alias_fingerprint_key_version', 'alias_fingerprint_schema_version') then
        ...
        elsif v_key = 'verify_result' then
            if jsonb_typeof(v_val) <> 'string' or (v_val #>> '{}') not in ('valid', 'invalid') then
                return true;
            end if;
...
comment on function public.audit_metadata_has_invalid_value_for_action_before_1330(text, text, jsonb) is
    'Returns true if metadata_json contains invalid values per action schema. Section 1360 adds entry_count (positive integer) and verify_result vocabulary validation for monthly_digest.';
