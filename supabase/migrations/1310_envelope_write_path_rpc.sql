...
alter table public.secret_versions
    drop constraint secret_versions_encrypted_data_key_length;

alter table public.secret_versions
    add constraint secret_versions_encrypted_data_key_length
    check (
        encrypted_data_key is null
        or octet_length(encrypted_data_key) = 73
    );

alter table public.secret_versions
    add constraint secret_versions_legacy_encrypted_data_key_required
    check (
        not (
            dek_wrap_algorithm is null
            or dek_wrap_algorithm = 'legacy-master-key-v1'
        )
        or encrypted_data_key is not null
    );

drop function public.rpc_write_secret_version(
    uuid,
    text,
    uuid,
    uuid,
...
create function public.rpc_write_secret_version(
    p_request_id uuid,
    p_action text,
    p_secret_id uuid,
    ...
    p_wrapped_dek bytea default null,
    p_dek_wrap_algorithm text default null,
    p_kek_version integer default null
)
returns table (
    secret_id uuid,
...
begin
    ...
    if p_dek_wrap_algorithm is not null
        and p_dek_wrap_algorithm not in (
    ...
    v_is_v02_envelope := p_dek_wrap_algorithm = 'envvar-xchacha-v2';

    if v_is_v02_envelope then
        if p_encrypted_data_key is not null
            or p_wrapped_dek is null
            or p_kek_version is null
            or p_kek_version <= 0
            or p_key_version <> p_kek_version
            or octet_length(p_wrapped_dek) not between 60 and 200
        then
            raise exception 'invalid_rpc_input' using errcode = '22023';
        end if;
    else
        if p_encrypted_data_key is null
            or octet_length(p_encrypted_data_key) <> 73
            or p_wrapped_dek is not null
            or p_kek_version is not null
        then
            raise exception 'invalid_rpc_input' using errcode = '22023';
        end if;
    end if;
...
comment on function public.rpc_write_secret_version(
    ...
    'Authoritative production write RPC for encrypt_create and encrypt_rotate. Inserts legacy v0.1.0 or envelope v0.2 secret versions, advances current_version_id, appends success audit events, and purges versions beyond retention in one transaction.';
