...
where sv.secret_id = s.id
    and sv.classification is null;

alter table public.secret_versions
    alter column classification set not null,
    add constraint secret_versions_classification_non_blank check (
        btrim(classification) <> ''
    ),
    add constraint secret_versions_classification_matches_aad check (
