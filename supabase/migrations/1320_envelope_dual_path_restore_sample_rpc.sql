drop function public.rpc_sample_restore_test(integer);

create function public.rpc_sample_restore_test(p_limit integer)
returns table (
    id uuid,
    secret_id uuid,
    version integer,
...
comment on function public.rpc_sample_restore_test(integer) is
    'Returns current encrypted rows for restore verification, including dual-path envelope fields. Service-role only; plaintext recovery remains outside Postgres. The sample limit is capped at 1000.';
