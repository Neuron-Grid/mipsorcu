            ...
            body: JSON.stringify({
                summary: {
                    ...
                    integrity_checks: [
                        {
                            ...
                            duration_ms: 21,
                            occurred_at: "2026-05-14T00:10:00Z",
                            result: "failure",
                            trigger: "integrity_scheduler",
                            violation_count: 1,
                        },
                    ],
                    ...
                    restore_tests: [
                        {
                            duration_ms: 34,
                            occurred_at: "2026-05-14T00:05:00Z",
                            result: "success",
                            sample_count: 4,
                            trigger: "restore_test_job",
                        },
                    ],
                    secret_count: 1,
                    sequence_end: 42,
                    sequence_start: 1,
                    signature_key_versions: [{ key_version: 1, status: "active" }],
...
test("read-only 監査データを表示し、検証失敗をハイライトし、禁止情報を表示しない", async ({
    page,
}) => {
    ...
    await expect(page.getByRole("heading", { name: "monthly digests" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "restore tests" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "integrity checks" })).toBeVisible();
    await expect(page.getByText("restore_test_job")).toBeVisible();
    await expect(page.getByText("integrity_scheduler")).toBeVisible();
    await expect(page.getByText("aa".repeat(32))).toBeVisible();
