...
export type AuditApiClient = {
  ...
  readonly verificationSummary: (periodStart: string, periodEnd: string) => Promise<SummaryResponse>;
};
...
const errorCodeFromResponse = async (response: Response): Promise<{ code: string; requestId: string | null }> => {
...
export const createAuditApiClient = (config: AppConfig, accessToken: string): AuditApiClient => ({
  ...
  integrityStatus: () => fetchReadOnlyJson<readonly IntegrityStatusRow[]>(config, accessToken, '/audit/v1/integrity-status'),
  verifyHashChain: () => fetchReadOnlyJson<HashChainVerification>(config, accessToken, '/audit/v1/verification/hash-chain'),
  verifySignatures: () => fetchReadOnlyJson<SignatureVerification>(config, accessToken, '/audit/v1/verification/signatures'),
