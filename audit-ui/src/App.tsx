...
import { createAuditApiClient, type AuditApiClient, AuditApiError } from './auditApi';
...
export const App = () => {
  ...
  if (session === null) {
    return <SignInPanel isLoading={authLoading} errorMessage={authError} onSignIn={handleSignIn} />;
  }
    ...
    <main class="shell">
      <header class="hero">
        <div>
          ...
          <h1>監査担当者向け台帳ビュー</h1>
          <p class="hero__description">SBC の read-only 監査 API から、非秘密メタデータと検証結果だけを表示します。</p>
        </div>
      ...
      <section class="controls" aria-label="照会条件">
        <label>
          対象期間開始
          <input value={periodStart} onInput={(event) => setPeriodStart(event.currentTarget.value)} />
        </label>
        <label>
          対象期間終了
          <input value={periodEnd} onInput={(event) => setPeriodEnd(event.currentTarget.value)} />
        </label>
        <label>
          digest 対象年月
          <input value={yearMonth} onInput={(event) => setYearMonth(event.currentTarget.value)} />
        </label>
        <button type="button" class="primary-button" onClick={loadDashboard} disabled={loadState === 'loading'}>
...
const VerificationCards = ({ hashChain, signatures, monthlyDigest, summary }: VerificationCardsProps) => (
  <section class="cards" aria-label="検証結果">
    <StatusCard
      title="hash chain"
      ok={hashChain.chain_valid}
      details={[`checked=${hashChain.entries_checked}`, nullableDetail('head', hashChain.chain_head_sequence_no)]}
      errorCode={hashChain.first_gap_detail ?? hashChain.first_hash_mismatch_detail}
    />
    <StatusCard
      title="signature"
      ok={signatures.valid}
      details={[`checked=${signatures.checked_count}`, rangeDetail(signatures.start_sequence_no, signatures.end_sequence_no)]}
...
const AuditEventsTable = ({ rows }: { readonly rows: readonly AuditEventRow[] }) => (
  <table>
    ...
    <tbody>
      {rows.map((row) => (
        <tr key={row.audit_event_id} class={row.result === 'failure' ? 'row--failure' : undefined}>
...
const LedgerEntriesTable = ({ rows }: { readonly rows: readonly LedgerEntryRow[] }) => (
  <table>
    ...
    <tbody>
      {rows.map((row) => (
        <tr key={row.ledger_entry_id} class={row.result === 'failure' || row.error_code !== null ? 'row--failure' : undefined}>
...
const rangeDetail = (start: number | null, end: number | null): string => `range=${start ?? '-'}..${end ?? '-'}`;